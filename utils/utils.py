...
def estimate_mean_stdev_with_bootstrap(X, n_bootstraps, bootstrap_size=400):
    ...
    X_bootstrap_sets = [bootstrap(X, bootstrap_size)
    ...
    err_mean = (pd.DataFrame(
        [(est_mean-x)**2 for x in means]).sum()/(len(X_bootstrap_sets)-1))**0.5
    err_stds = (pd.DataFrame(
        [(est_stdev-x)**2 for x in stdevs]).sum()/(len(X_bootstrap_sets)-1))**0.5
...
def print_histograms(data, subtitles=None, title='insert_title', figsize=(10, 5)):
    ...
    # count the number of wines per quality
    values = [len(x[x == i]) for i in possible_qualities]
...
def plot_explained_variance(explained_variance, title):
    ...
    plt.axvline(x=x_90th, marker='.', ls='--', color='blue',
                label='90% variance or grater')
    plt.legend()
    plt.show()
    return x_90th
...
def plot_decision_boundaries(ax, clf, xx, yy, n_features, **params):
    ...
    :return: contours
    """
    Xpred = np.array([xx.ravel(), yy.ravel()] + [np.repeat(0,
                     xx.ravel().size) for _ in range(n_features-2)]).T
    Z = clf.predict(Xpred)
...
def decision_boundaries(X_test, y_test, str, clf):
    ...
    plot_decision_boundaries(axes, clf, xx, yy, n_features=len(
        X_test.columns), cmap=plt.cm.coolwarm, alpha=0.8)
